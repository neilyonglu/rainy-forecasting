...
# -----------------------------
def mock_rain_payload(lat=23.97, lon=120.98):
    return {
...
if mode == 0:  # Home
    ...
    # 示意假定位（之後可改為瀏覽器 geolocation 注入）
    lat, lon = 23.9739, 120.9820
    st.caption(f"模擬位置：緯度 {lat}, 經度 {lon}")

    data = mock_rain_payload(lat, lon)
    show_result_card(data)
